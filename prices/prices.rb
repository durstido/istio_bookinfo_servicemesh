...
server.mount_proc '/health' do |req, res|
    res.status = 200
    res.body = {'status' => 'Prices is healthy'}.to_json
    res['Content-Type'] = 'application/json'
end

server.mount_proc '/prices' do |req, res|
    ...
    begin
        ...
        prices = get_book_prices(id, headers)
        res.body = prices.to_json
        res['Content-Type'] = 'application/json'
    rescue => error
...
# TODO: provide prices on different books.
def get_book_prices(id, headers)
    if ENV['ENABLE_EXTERNAL_BOOK_SERVICE'] === 'true' then
      ...
      # that has Shakespeare as the single author
        isbn = '0486424618'
        return fetch_prices_from_external_service(isbn, id, headers)
    end
...
def fetch_prices_from_external_service(isbn, id, headers)
    uri = URI.parse('https://www.googleapis.com/books/v1/volumes?q=isbn:' + isbn)
    http = Net::HTTP.new(uri.host, ENV['DO_NOT_ENCRYPT'] === 'true' ? 80:443)
    http.read_timeout = 5 # seconds

    # DO_NOT_ENCRYPT is used to configure the prices service to use either
