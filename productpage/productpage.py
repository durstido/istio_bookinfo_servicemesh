...
pricesHostname = "prices" if (os.environ.get("PRICES_HOSTNAME") is None) else os.environ.get("PRICES_HOSTNAME")
...
prices = {
    "name": "http://{0}{1}:9080".format(pricesHostname, servicesDomain),
    "endpoint": "prices",
    "children": []
}
...
def getProductPrices(product_id, headers):
    try:
        url = prices['name'] + "/" + prices['endpoint'] + "/" + str(product_id)
        res = requests.get(url, headers=headers, timeout=3.0)
    except BaseException:
        res = None
    if res and res.status_code == 200:
